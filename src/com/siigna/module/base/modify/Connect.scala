    ...
    'Start -> {
      ...
      case MouseDown(p, MouseButtonLeft, modifier) :: tail => {
        ...
        if (Drawing.selection.isEmpty) {
          println("YES")
          Start('Selection, "com.siigna.module.base", MouseDown(p, MouseButtonLeft, modifier)) //look for one more shape to connect to
        }
      ...
      case _ => {
        ...
        if(Drawing.selection.size == 1) {
          ...
          shapeID1 = Some(Drawing.selection.head.shapes.head._1)
          //attr = shape1.get.attributes //get attributes of the first shape to be transferred to the second.
          //Start('Selection, "com.siigna.module.base") //look for one more shape to connect to
