...
case class Properties(parent : Option[MenuCategory]) extends MenuCategory {
  ...
  override def NNE = Some(MenuItem('AntiAliasingToggle, TextShape("Anti-Alias", Vector(0, 0), 10, Attributes("TextAlignment" -> Vector(0.5, 0.5)))))
  override def N   = Some(MenuItemEmpty("Weight"))
  override def NNW = Some(MenuItem('Polyline, RadialMenuIcon.NNWArrow))
  override def WNW = Some(MenuItem('Polyline, TextShape("mm", Vector(0, 0), 10, Attributes("TextAlignment" -> Vector(0.5, 0.5)))))
  override def WSW = Some(MenuItem('Polyline, TextShape("Feet", Vector(0, 0), 10, Attributes("TextAlignment" -> Vector(0.5, 0.5)))))
