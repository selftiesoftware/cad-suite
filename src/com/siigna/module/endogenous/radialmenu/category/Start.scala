...
case object Start extends MenuCategory {
  ...
  override def NNW = Some(new MenuItem('Text, RadialMenuIcon.text))
  override def WNW = Some(new MenuItem('ColorWheel, RadialMenuIcon.colorWheel))
  override def WSW = Some(new MenuItem('Polyline, RadialMenuIcon.sampleProperties))
  override def SSW = Some(new MenuItem('Polyline, RadialMenuIcon.rotate))
