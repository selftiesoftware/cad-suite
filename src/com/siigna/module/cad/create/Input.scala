    ...
    'Start -> {
      ...
      case KeyDown(key,modifier) :: tail => {
        ...
        //BACKSPACE with no modifiers: Is returned to the asking module as a key-down event:
          ...
          if (drawGuide == true) drawGuide = false
          if (!inputRequest.isEmpty) Start('cad,"create.InputTwoValues",inputRequest.get)
          else Start('cad,"create.InputTwoValues")
          ...
          || inputType == Some(120)) {
          if (drawGuide == true) drawGuide = false
          if (!inputRequest.isEmpty) Start('cad,"create.InputOneValue",inputRequest.get)
          else Start('cad,"create.InputOneValue")
        } else if(inputType == Some(14) ) {
          if (drawGuide == true) drawGuide = false
          if (!inputRequest.isEmpty) Start('cad,"create.InputText",inputRequest.get)
          else Start('cad,"create.InputText")
        }
      }
